...
ApplicationSettings::ApplicationSettings(const String& commandLine)
{
...
void ApplicationSettings::Initialize(const Ogre::String& commandLine)
{
    CommandLineArgs commandLineArgs(commandLine);
    Initialize(commandLineArgs);
}

void ApplicationSettings::Initialize(int argc, char* argv[])
{
    CommandLineArgs commandLineArgs(argc, argv, true);
    Initialize(commandLineArgs);
}
...
void ApplicationSettings::Initialize(const CommandLineArgs& commandLine)
{
    ...
    if (commandLine.GetLength() > 0)
    {
        ...
        for (int index = 0; index < commandLine.GetLength(); index++)
        {
            ...
            if (argLower == "-direct3d" || argLower == "-d3d")
                this->renderingSubsystem = DIRECT3D;
...
{
    RenderSystemList* renderSystems = Root::getSingleton().getAvailableRenderers();

    for (RenderSystemList::iterator renderSystem = renderSystems->begin();
